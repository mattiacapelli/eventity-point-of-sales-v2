...
export const paymentsModule: PosModule = {
  ...
  async init(ctx: CoreContext) {
    ...
    _service = new PaymentService(repo, ctx.eventBus, ctx.db);
  ...
  async register(ctx: CoreContext) {
    ...
    const svc = _service;
    const guard = fastify.moduleGuard?.("payments");
    await fastify.register(async (f) => { registerPaymentRoutes(f, svc, guard); }, { prefix: "/api" });
  },
